...
export async function getDashboardStats() {
    ...
    try {
        const [postsResult, categoriesResult, tagsResult] = await Promise.all([
            db
                .prepare(
                    'SELECT COUNT(*) as total, SUM(CASE WHEN published = 1 THEN 1 ELSE 0 END) as published FROM blog_posts'
                )
                .first(),
        ...
        return {
            ...
            draftPosts:
                ((postsResult as any)?.total || 0) -
                ((postsResult as any)?.published || 0),
...
export async function createCategory(
    ...
    await db
        ...
        .bind(data.name, data.slug, data.description || null, data.sort_order)
        .run();
...
export async function createTag(data: Omit<Tag, 'id'>): Promise<Tag> {
    ...
    await db
        ...
        .bind(data.name, data.slug, data.description || null)
        .run();
...
export async function updatePost(
    slug: string,
    data: Partial<Omit<PostWithRelations, 'slug' | 'tags'>> & {
        tag_ids?: number[];
    }
): Promise<PostWithRelations> {
    const db = getDB();
    ...
    if (data.tag_ids !== undefined) {
        // 删除旧标签
        await db
            .prepare('DELETE FROM post_tags WHERE post_slug = ?')
            .bind(slug)
            .run();
        // 添加新标签
