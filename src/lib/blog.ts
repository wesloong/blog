...
const fallbackPosts: BlogPost[] = [
  ...
  },
  {
    ...
    date: '2024-01-02',
    excerpt: '介绍如何使用 Next.js 14 和 Material-UI 5 搭建一个现代化的博客系统。',
  ...
  },
  {
    ...
    featured_image: 'https://images.unsplash.com/photo-1516116216624-53e6977beab8?w=800&h=400&fit=crop',
    view_count: 89,
    content: `
      <h2>类型推断</h2>
...
export async function getBlogPosts(): Promise<BlogPost[]> {
  ...
  try {
    ...
    console.error('Error fetching blog posts from D2:', error)
    return fallbackPosts
  }
}
...
export async function getBlogPost(slug: string): Promise<BlogPost | null> {
  ...
  try {
    ...
    console.error('Error fetching blog post from D2:', error)
...
export async function getAdjacentPosts(slug: string): Promise<{ prev: BlogPost | null; next: BlogPost | null }> {
  ...
  try {
    ...
    ).bind(currentPost.date).first<{ slug: string; title: string }>()
    ...
    return {
      prev: prevResult ? { slug: prevResult.slug, title: prevResult.title } as BlogPost : null,
      next: nextResult ? { slug: nextResult.slug, title: nextResult.title } as BlogPost : null,
    }
  } catch (error) {
...
export async function getRelatedPosts(slug: string, limit: number = 3): Promise<BlogPost[]> {
  ...
  try {
    ...
    ).bind(slug).first<{ category_id?: number; tag_ids?: string }>()
